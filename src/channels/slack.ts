...
registerChannelAdapter('slack', {
  factory: () => {
    ...
    const bridge = createChatSdkBridge({
      ...
      // Socket Mode: events arrive over the adapter's outbound WebSocket, so
      // skip the shared inbound webhook server (keeps port 3000 free for other
      // local apps). Restores the intent of our fork's original 6eb9b16.
